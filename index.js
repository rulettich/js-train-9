...
function findIndexByProperty(arr, name) {
  ...
  const arrInd = arr.findIndex((obj) => obj.name === name);
  return arrInd;
}
...
function getPositiveArrayValues(arr) {
  ...
  const itArr = Array.from(iter);
  const positiveArrayValues = itArr.filter((value) => value > 0);
...
function capitalizeWords(arr) {
  ...
  const capitalizedArray = arr.map((word) => {
    if (word && word.length > 0) {
      // Перетворюємо першу літеру слова у верхній регістр і додаємо до неї всі символи слова крім першого
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((acc, currentItem) => {
